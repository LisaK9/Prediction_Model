...
for model_name, model in models.items():
        ...
        for fs_name, selector in feature_selectors.items(): #Feature Selektion
            ...
            for n_features in range(1, len(features) + 1): #Jede Anzahl an Features durchlaufen
                ...
                #Prüfen ob enthalten in param_grid, wenn ja, führe Hyperparameter-Tuning durch
                if model_name in param_grid:
                    ...
                    # Vergleich ohne/mit Tuning: Behalte das Modell mit dem kleineren MSE
        ...
        residuals_dict[key] = current_residuals
...
plt.xticks(rotation=45, ha='right')  # Modellnamen drehen für bessere Lesbarkeit
plt.tight_layout()  # Layout anpassen, um Überlappungen zu vermeiden
modeling_functions.save_plot("MSE-Vergleich.png")
plt.show()


#Bestes Modell anhand MSE auswählen
best_model_key = min(results, key=lambda k: results[k]['MSE'])
print(f"Bestes Modell basierend auf MSE: {best_model_key} mit MSE: {results[best_model_key]['MSE']:.4f}")
best_model = best_estimators[best_model_key]
print(best_model)

#Bestes Modell, Scaler und verwendete Features als pkl Datei speichern
modeling_functions.save_model(best_model,'best_model.pkl')
modeling_functions.save_model(model_best_combinations[best_model_key]['selected_features'],'selected_features.pkl')
modeling_functions.save_model(best_scalers[best_model_key],'scaler.pkl')